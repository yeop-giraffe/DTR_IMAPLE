...
def detect(weights=ROOT / 'yolov5n.pt',
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        # We only run YOLO each 'n' iterations
        #if True: #frame_idx % n == 0:
        ...
        if frame_idx % n_frames == 0:
            ...
            t3 = time_sync()
            dt[1] += t3 - t2
            print(f'Time to run Inference {dt[1]}, {frame_idx}, {frame_idx % n_frames}')

            # NMS
            pred = non_max_suppression(pred, conf_thres, iou_thres, classes, agnostic_nms, max_det=max_det)
            dt[2] += time_sync() - t3
            #else:
            #    # Else we use the OpenCV Tracker to update the bounding boxes
            #    pass
            ...
            # Restarts the OpenCV Multi-tracker
            multi_tracker = None
        else:
            # If we do not run YOLO
            ...
            # Run SORT
            dets_to_sort = np.array([[b[0], b[1], b[0] + b[2], b[1] + b[3], c, cl] for b, c, cl in zip(boxes, confs, ids)])
            tracked_dets = sort_tracker.update(dets_to_sort)
            tracks = sort_tracker.getTrackers()
            ...
            if len(tracked_dets) > 0:
                ...
                identities = tracked_dets[:, 8]
                categories = tracked_dets[:, 4]
                draw_boxes(image_to_show, bbox_xyxy, identities, categories, names, color_box)

        if view_img:
            cv2.imshow(str(Path('0')), image_to_show)
            cv2.waitKey(1)
